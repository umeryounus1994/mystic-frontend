...
export class EditQuestComponent implements OnInit {
  ...
  constructor(private api: RestApiService, private sp: NgxSpinnerService, private helper: HelperService,
    ...
    private questUpload: QuestUploadService, private translate: TranslateService) {
  ...
  async _sendSaveRequest(formData: any) {
    ...
    try {
      ...
      await this.api.postImageData('quest/updateQuest/' + this.Id, fD);
...
openExternalFile(fileUrl: string) {
  ...
  if (this.isAndroidDevice() && this.isPdfFile(url)) {
    // Android browsers/webviews often block pdf popups/download attribute.
