  ...
  _sendSaveRequest(formData: any) {
    ...
    this.api.postImageData('activity', fD)
      .then((response: any) => {
        this.sp.hide();
        setTimeout(() => {
          this.helper.successToast(this.translate.instant('MESSAGES.ACTIVITY_CREATED_SUCCESS'));
        }, 1000);
        setTimeout(() => {
      ...
      .catch((error: any) => {
        this.sp.hide();
        this.helper.failureToast(error?.error?.message || this.translate.instant('MESSAGES.FAILED_TO_CREATE_ACTIVITY'));
      });
  }
}
